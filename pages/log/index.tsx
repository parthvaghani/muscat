...
export default function LogPage() {
  ...
  const handleFilter = () => {
    let filteredItems = totalItems.filter((item: LogItem) => {
      if (!filterOptions.userLogin && item.log_type === "사용자 로그인")
        return false;
      if (!filterOptions.adminLogin && item.log_type === "관리자 로그인")
        return false;
      ...
      if (!filterOptions.accessLog && item.log_type === "접속기록")
        return false;
      return true;
    });
    ...
    if (searchItem === "전체") {
      ...
      setLogItems(
        filteredItems.filter((x: LogItem) => x.api_name.includes(searchName))
      );
    } else if (searchItem === "이름") {
      setLogItems(
        filteredItems.filter((x: LogItem) => x.user_name.includes(searchName))
      );
    } else if (searchItem === "이메일") {
      setLogItems(
        filteredItems.filter((x: LogItem) => x.user_email.includes(searchName))
      );
    } else if (searchItem === "접속 IP") {
      setLogItems(
        filteredItems.filter((x: LogItem) => x.ip_address.includes(searchName))
      );
    } else {
      setLogItems(
        filteredItems.filter((x: LogItem) => x.log_type.includes(searchName))
      );
    }
  };
                    ...
                    <TableRow>
                      <TableCell colSpan={8}>
                        ...
                        >
                          <Box sx={{ padding: 2 }}>
                            <Typography>
                              로그 요청: {row.log_request}
                            </Typography>
                            <Typography>
                              로그 응답: {row.log_response}
                            </Typography>
                          </Box>
                        </Collapse>
                      </TableCell>
