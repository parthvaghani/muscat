...
export default function MemoTable() {
  ...
  const fetchMemoDetails = async (memo_id: string) => {
    const response = await axiosPost(`${API_URL}/memo_details/List`, {
      memo_id: memo_id,
    });
    setShowDetail(true);
  ...
  const handleSearch1 = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.value == "전체") {
      ...
      const datas = totalDatas.filter((row: any) => {
        return (
          row.reason!.toString().toLowerCase().includes(event.target.value) &&
    ...
    const datas = totalDatas.filter((row: any) => {
      return (
        row
          .reason!.toString()
          .toLowerCase()
          .includes(search1 == "전체" ? "" : search1) &&
        row
          .consignor_name!.toString()
          .toLowerCase()
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - filterDatas.length) : 0;
