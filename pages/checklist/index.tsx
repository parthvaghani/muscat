...
export default function ChecklistManagement() {
  ...
  const handleDeleteSelectedItems = async () => {
    try {
      const response = await axiosDelete(`${API_URL}/checklist/Delete`, {
    ...
    <>
      {showCheckInfo == false ? (
        <Box sx={{ width: "100%" }}>
          <Breadcrumb
            title="점검 항목 관리"
            items={[{ to: "/", title: "메인" }, { title: "점검 항목 관리" }]}
          />
          <Toolbar
            sx={{
              pl: { sm: 2 },
                ...
                <TableRow>
                  <TableCell padding="checkbox">
                    <Checkbox
                      indeterminate={
                        selected.length > 0 &&
                        selected.length < checklistItems.length
                      }
                      checked={
        ...
        <CheckInfoTable
          initChecklistItems={checklistItems}
          selectedItem={selectCheckItem}
          onClose={() => {
