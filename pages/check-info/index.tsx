...
const CheckInfoTable: React.FC<CheckProps> = ({
  selectedItem,
  initChecklistItems,
  onClose,
}) => {
  const [checklistItems, setChecklistItems] =
    useState<ChecklistItem[]>(initChecklistItems);
  const [checklistImport, setChecklistImport] =
    useState<ChecklistItem>(selectedItem);
  ...
  const handleAddRow = () => {
    ...
    if (selectedCell) {
      ...
      if (selectedCell.colIndex == 2) {
        for (
          let i = selectedCell.rowIndex + 1;
          i < checkInfos.length + 1;
          i++
        ) {
      ...
      const updatedRows = checkInfos.map((row, rowIndex) => {
        if (rowIndex === selectedCell.rowIndex) {
          ...
          return {
            ...row,
            merged1: 0,
            area: checkInfos[selectedCell.rowIndex].area,
          };
        }
        return row;
      });
      ...
      const updatedRows = checkInfos.map((row, rowIndex) => {
        if (rowIndex === selectedCell.rowIndex) {
          ...
          return {
            ...row,
            merged2: 0,
            domain: checkInfos[selectedCell.rowIndex].domain,
          };
        }
        return row;
      });
    ...
    if (files) {
      ...
      const updatedFiles: SelectedFile[] = selectedFiles.map((selectedFile) => {
        if (selectedFile.id === inputId) {
          return (
            newFiles.find((newFile) => newFile.id === inputId) || selectedFile
          );
        }
        return selectedFile;
      });
  ...
  return (
    <>
      ...
      <Box sx={{ mb: 2, display: "flex", justifyContent: "space-between" }}>
        <Button sx={{ width: 100 }} variant="contained" onClick={onClose}>
          목록
        </Button>
        <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 1 }}>
          ...
          <Button
            ...
            disabled={
              selectedCell && checkInfos.length > selectedCell.rowIndex
                ? checkInfos[selectedCell.rowIndex].merged1 !== 1 ||
                  checkInfos[selectedCell.rowIndex].merged2 !== 1
                : true
            }
          >
            행 삭제
                    ...
                    >
                      {isEditing ? (
                        <TextField
                          value={row.item}
                          onChange={(e) =>
                            handleCellUpdate(rowIndex, "item", e.target.value)
                          }
                          onKeyDown={(e: any) => {
                            if (e.key === "Enter") {
                              e.target.blur();
      ...
      <Box display={"flex"} justifyContent={"flex-end"} marginTop={2}>
        <Button variant="contained" onClick={handleSave} disabled={!willSave}>
          저장
        </Button>
      </Box>
    </>
