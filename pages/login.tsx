...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      response = {};
      if (authSent) {
        response = await axiosPost(`${API_URL}/user/Login`, {
          email: email,
          password: password,
          code: validCode,
        });
      ...
      if (response.loginResult === "send email") {
        ...
        setErrorMessage(
          `입력하신 정보가 맞지 않습니다.(남은 횟수: ${response.remainCnt}번)`
        );
      } else if (response.loginResult == "locked user") {
        setErrorMessage(
          `로그인 실패 5회로 10분 동안 로그인할 수 없습니다. ${Math.trunc(
            response.after / 60
          )}분 ${Math.trunc(response.after) % 60}초 뒤 다시 시도하세요.`
        );
      } else if (response.loginResult == "invalid code") {
        setErrorMessage(`인증 번호가 정확하지 않습니다.`);
      } else if (response.loginResult == "code expired") {
        setErrorMessage(
          `인증 번호 입력시간이 경과했습니다. 다시 로그인해 주세요.`
        );
        setAuthSent(false);
    ...
    <Box>
      ...
      >
        <Link
          href="/introduction"
          underline="none"
          sx={{ color: "white", fontSize: 16, ml: 3 }}
        >
          회사소개
        </Link>
