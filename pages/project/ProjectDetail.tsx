...
const ProjectDetail = ({ setMode, data }: { setMode: any; data: any }) => {
  ...
  const fetchData = async () => {
    ...
    if (data.company.length > 0) setCompany(data.company[0].id);

    setAdminList(response.data.admin);
    if (data.admin.length > 0) setChecker(data.admin[0].user_id);
  };
  //Fetch Products
  React.useEffect(() => {
    fetchData();
  ...
  return (
    <Box>
      ...
      <Box sx={{ mb: 2, display: "flex", alignItems: "center" }}>
        ...
        <Typography
          align="center"
          sx={{ mr: 3, fontSize: 18, fontWieght: "bold" }}
        >
          {data.year}
        </Typography>
        <Typography align="center" sx={{ mr: 1, fontSize: 16 }}>
      ...
      <BlankCard>
        ...
        >
          ...
          <Button
            ...
            sx={{ width: 80, ml: 1 }}
            onClick={() => {
              if (editMode == "register") setEditMode("list");
                  ...
                  <TableRow hover role="checkbox" tabIndex={-1}>
                    ...
                    <CustomTableCell>
                      ...
                      >
                        {adminList.map((x, i) => {
                          return (
                            <MenuItem value={x.user_id} key={i}>
                              {x.name}
                            </MenuItem>
                          );
                        })}
