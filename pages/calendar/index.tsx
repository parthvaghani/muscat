...
export default function BigCalendar() {
  ...
  useEffect(() => {
      ...
      if (projectDetail.set) {
        ...
        if (len > 0) {
          for (let i = 0; i < len; i++) {
            ...
            if (i == 0) startDate = schedule.create_to;
            else startDate = delay.create[i - 1];
        ...
        if (len > 0) {
          for (let i = 0; i < len; i++) {
            ...
            if (i == 0) startDate = schedule.self_check_to;
            else startDate = delay.self_check[i - 1];
        ...
        if (len > 0) {
          for (let i = 0; i < len; i++) {
            ...
            if (i == 0) startDate = schedule.imp_check_to;
            else startDate = delay.imp_check[i - 1];
    ...
    if (
      getDateStr(first_check) > schedule.self_check_to &&
      getDateStr(first_check) < schedule.imp_check_from
    ) {
      newEvents.push({
        title: "1차 검수 기간",
        allDay: true,
  ...
  useEffect(() => {
    fetchSchedule();
    if (userData.type != 1) fetchConsignee();
    fetchCheckSchedule();
    if (userData.type == 1) fetchDetail();
  }, [project]);
  ...
  const fetchCheckSchedule = async () => {
    if (project) {
      ...
      let newData: {
        ...
        user_name: any;
        admin_name: any;
        data: { time: any; address: any; manager: any; phone: any };
      }[] = [];
  ...
  const fetchAdmins = async () => {
    ...
    if (data.admin.length > 0) setAdmin(data.admin[0].user_id);
  };
  ...
  const addNewEventAlert = (slotInfo: EvType) => {
    if (
      ...
      project &&
      consignee &&
      (userData.type == 0 || userData.type == 3)
    ) {
    ...
    (date: Date) => {
      ...
      if (dateStr >= schedule.create_from && dateStr <= schedule.create_to) {
        return {
          style: {
            backgroundImage:
              "linear-gradient(to bottom right, #ffff85, #ffffda)",
          },
        };
      } else if (
        dateStr >= schedule.self_check_from &&
        dateStr <= schedule.self_check_to
      ) {
        return {
          style: {
            backgroundImage:
              "linear-gradient(to bottom right, #ffc7b1, #fbe5d6)",
          },
        };
      } else if (
        dateStr >= schedule.imp_check_from &&
        dateStr <= schedule.imp_check_to
      ) {
        return {
          style: {
            backgroundImage:
              "linear-gradient(to bottom right, #c4e3b6, #e6f2e0)",
          },
        };
      } else if (
        dateStr > schedule.self_check_to &&
        ...
        return {
          style: {
            backgroundImage:
              "linear-gradient(to bottom right, #b6b3b3, #e8e7e7)",
          },
        };
      }
    },
  ...
  const onSaveCheck = async () => {
    if (checkMode == "register") {
      ...
      for (let i = 0; i < checkSchedule.length; i++) {
        if (
          checkSchedule[i].company_id == consignee &&
          checkSchedule[i].project_id == project &&
          checkSchedule[i].checker_id == admin
        ) {
      ...
      for (let i = 0; i < newSchedule.length; i++) {
        if (
          newSchedule[i].user_id == newSchedule[editIndex].user_id &&
          newSchedule[i].project_id == newSchedule[editIndex].project_id &&
          newSchedule[i].checker_id == newSchedule[editIndex].checker_id
        ) {
  ...
  return (
    <PageContainer>
        ...
        {userData.type != 1 && (
          <>
            ...
            >
              {consignees.map((x, i) => {
                return (
                  <MenuItem key={i} value={x.company_id}>
                    {x.name}
                  </MenuItem>
                );
              })}
          ...
          <DialogContent>
            ...
            <Box sx={{ display: "flex", alignItems: "center", mb: 1 }}>
              ...
              <Box sx={{ flex: 1, display: "flex" }}>
                {checkMode == "view" ? (
                  <Typography>{checkAddress}</Typography>
                ) : (
                  <TextField
                    placeholder=""
                    size="small"
            ...
            <Box sx={{ display: "flex", alignItems: "center", mb: 1 }}>
              ...
              <Box sx={{ flex: 1, display: "flex" }}>
                {checkMode == "view" ? (
                  <Typography>{checkManager}</Typography>
                ) : (
                  <TextField
                    placeholder=""
                    size="small"
            ...
            <Box sx={{ display: "flex", alignItems: "center" }}>
              ...
              <Box sx={{ flex: 1, display: "flex" }}>
                {checkMode == "view" ? (
                  <Typography>{checkManagerPhone}</Typography>
                ) : (
                  <TextField
                    placeholder=""
                    size="small"
                    onChange={(e: any) => {
                      setCheckManagerPhone(e.target.value);
                    }}
      ...
      <Dialog open={deleteModal} onClose={onCloseDeleteModal}>
        ...
        <DialogContent>
          <DialogContentText>
            선택한 일정연기부분을 삭제하시겠습니까? 이 작업은 되돌릴 수
            없습니다.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
