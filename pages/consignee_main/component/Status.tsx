...
const StatusLayout = ({ project, setNoStatus }: Props) => {
  ...
  const onClose1 = () => {
    setShowModal1(false);
    if (modalMsg1 == "정확히 저장되었습니다.") setNoStatus(false);
  };
      ...
      <Grid container spacing={3}>
        ...
        <Grid item sm={9}>
          <CustomOutlinedInput
            ...
            startAdornment={
              <InputAdornment position="start">총</InputAdornment>
            }
        ...
        <Grid item xs={12} sm={3} display="flex" alignItems="start">
          <CustomFormLabel
            htmlFor="privacy-item-select-label"
            sx={{ mt: 0, mb: { xs: "-10px", sm: 0 } }}
          >
            연간 개인정보 처리량*
          </CustomFormLabel>
        ...
        <Grid item xs={12} sm={9}>
          <RadioGroup
            row
            name="delivery-opt"
            value={annualPersonalInformation}
            onChange={(event) =>
              setAnnualPersonalInformation(event.target.value)
            }
          >
            <FormControlLabel
              value="<1000"
          ...
          <CustomOutlinedInput
            ...
            startAdornment={
              <InputAdornment position="start">연간</InputAdornment>
            }
            endAdornment={<InputAdornment position="end">명</InputAdornment>}
            inputProps={{ style: { textAlign: "end" } }}
          />
        ...
        >
          <RadioGroup
            row
            name="system-opt"
            value={systemUsageStatus}
            onChange={(event) => setSystemUsageStatus(event.target.value)}
          >
            <FormControlLabel
              value="no"
              control={<CustomRadio />}
          ...
          {systemUsageStatus === "yes" && (
            <CustomTextField
              ...
              placeholder="시스템 명"
              sx={{ width: 600 }}
              value={systemUsageStatusText}
              onChange={(e: any) => setSystemUsageStatusText(e.target.value)}
            />
          )}
        </Grid>
        <Grid item xs={12} sm={3} display="flex" alignItems="center">
        ...
        >
          <RadioGroup
            ...
            name="reconsign-opt"
            value={retrustStatus}
            onChange={(event) => setRetrustStatus(event.target.value)}
          >
            <FormControlLabel
              value="no"
              control={<CustomRadio />}
          ...
          {retrustStatus === "yes" && (
            <CustomTextField
              id="bl-re"
              sx={{ width: 600 }}
              placeholder="재위탁사 명(위탁업무)"
              value={retrustStatusText}
              onChange={(e: any) => setRetrustStatusText(e.target.value)}
            />
          )}
        </Grid>
        <Grid item xs={12} sm={3} display="flex" alignItems="center">
        ...
        >
          <RadioGroup
            ...
            onChange={(event) => setThirdPartyStatus(event.target.value)}
          >
            <FormControlLabel
              value="no"
              control={<CustomRadio />}
          ...
          {thirdPartyStatus === "yes" && (
            <CustomTextField
              id="bl-thid"
              placeholder="제3자제공 업체 명(제공내역)"
              sx={{ width: 600 }}
              value={thirdPartyStatusText}
              onChange={(e: any) => setThirdPartyStatusText(e.target.value)}
            />
          )}
        </Grid>
        <Grid item xs={12}>
              ...
              return (
                <Grid container spacing={0} key={i}>
                  ...
                  <Grid
                    ...
                    display="flex-wrap"
                    alignItems="center"
                    sx={{ border: 1, borderLeft: 0, borderTop: i ? 0 : 1 }}
                  >
                    {x.data.map((a: any, ind: any) => {
                      let selected =
                        selectedItems.findIndex((b) => b.id == a.id) != -1;
                      return (
                        <Typography
                          key={ind}
                          onClick={() => {
                            onItemClick(a.id, a.name);
                          }}
                          sx={{ color: selected ? "blue" : "black" }}
                        >
                          {a.name}
                        </Typography>
                      );
