...
export default function MainPage() {
  ...
  const [representativeIndustry, setRepresentativeIndustry] =
    useState<string>("");
  ...
  const [annualPersonalInformation, setAnnualPersonalInformation] =
    useState<string>("<1000");
  const [systemUsageStatus, setSystemUsageStatus] = useState<string>("no");
  const [systemUsageStatusText, setSystemUsageStatusText] =
    useState<string>("");
  ...
  const [personInChargeContact, setPersonInChargeContact] =
    useState<string>("");
  ...
  const fetchDetail = async () => {
    ...
    if (delay) delay = JSON.parse(delay);
    else
      delay = {
        create: [],
        self_check: [],
    ...
    setIssueCount1(
      response.data.filter((x: any) => x.state == 1 && x.issue_id != null)
        .length
    ); //수정필요
    setIssueCount2(
      response.data.filter((x: any) => x.state == 2 && x.issue_id != null)
        .length
    ); //수정필요
    setDelayCount1(
      response.data.filter((x: any) => x.state == 1 && x.delay != null).length
    ); //수정필요
    setDelayCount2(
      response.data.filter((x: any) => x.state == 2 && x.delay != null).length
    ); //수정필요
    ...
    setPrepareCount1(
      response.data.filter(
        (x: any) => x.state == 1 && x.sub_state == 1 && x.issue_id == null
      ).length
    );
    setPrepareCount2(
      response.data.filter(
        (x: any) => x.state == 2 && x.sub_state == 1 && x.issue_id == null
      ).length
    );
    setCheckingCount1(
      response.data.filter(
        (x: any) => x.state == 1 && x.sub_state == 2 && x.issue_id == null
      ).length
    );
    setCheckingCount2(
      response.data.filter(
        (x: any) => x.state == 2 && x.sub_state == 2 && x.issue_id == null
      ).length
    );
    setCompleteCount1(
      response.data.filter(
        (x: any) => x.state == 1 && x.sub_state == 3 && x.issue_id == null
      ).length
    );
    setCompleteCount2(
      response.data.filter(
        (x: any) => x.state == 2 && x.sub_state == 3 && x.issue_id == null
      ).length
    );
    ...
    setTotalCompCount1(
      response.data.filter(
        (x: any) => x.state == 1 && x.sub_state == 3 && x.issue_id == null
      ).length + response.data.filter((x: any) => x.state == 3).length
    );
    setTotalCompCount2(
      response.data.filter(
        (x: any) => x.state == 2 && x.sub_state == 3 && x.issue_id == null
    ...
    setCheckingDetail(
      response.data.filter(
        (x: any) =>
          (x.state == 1 && x.sub_state == 3) ||
          (x.state == 2 && x.sub_state == 3)
      )
    );
    setScheduleCheckDetail(
      response.data.filter((x: any) => x.state < 3 && x.sub_state < 3)
    );
  };
  ...
  const fetchProjectsByConsignee = async (id: any, notices: any) => {
    ...
    const noticeFilter = notices.filter((x: any) =>
      projects.some(
        (project: any) =>
          project.project_id === x.project_id || "전체" === x.project_name
      )
    );
    ...
    return (
      ...
      >
        ...
        <Collapse in={open} timeout="auto" unmountOnExit>
          <StatusItem
            label="계약종료 확인필요"
            count={issueDetail.filter((x: any) => x.issue_type == 0).length}
            bgcolor={issue_type == 0 ? "lightblue" : "lightgrey"}
            onClick={() => setIssueType(0)}
          />
          <Divider flexItem />
          <StatusItem
            label="담당자 연락불가"
            count={issueDetail.filter((x: any) => x.issue_type == 1).length}
            bgcolor={issue_type == 1 ? "lightblue" : "lightgrey"}
            onClick={() => setIssueType(1)}
          />
          <Divider flexItem />
          <StatusItem
            label="점검거부"
            count={issueDetail.filter((x: any) => x.issue_type == 2).length}
            bgcolor={issue_type == 2 ? "lightblue" : "lightgrey"}
            onClick={() => setIssueType(2)}
          />
          <Divider flexItem />
          <StatusItem
            label="기타"
            count={issueDetail.filter((x: any) => x.issue_type == 3).length}
            bgcolor={issue_type == 3 ? "lightblue" : "lightgrey"}
            onClick={() => setIssueType(3)}
          />
          <Typography variant="h6" sx={{ mt: 1 }}>
            총{" "}
            {issueDetail.filter((x: any) => x.issue_type == issue_type).length}{" "}
            건
          </Typography>
          <List>
            {issueDetail
              .filter((x: any) => x.issue_type == issue_type)
              .map((item, index) => (
                <ListItem
                  key={index}
                  button
                  onClick={async () => {
                    const response = await axiosPost(
                      `${API_URL}/memo_details/List`,
                      { memo_id: item.issue_id }
                    );
  ...
  const DelayCard: React.FC<{ delayCount1: number; delayCount2: number }> = ({
    delayCount1,
    delayCount2,
  }) => {
    const [open, setOpen] = React.useState(true);
      ...
      >
        ...
        <Collapse in={open} timeout="auto" unmountOnExit>
          <StatusItem
            label="계정 미생성"
            count={0}
            bgcolor={delay_type == 0 ? "lightblue" : "lightgrey"}
            onClick={() => setDelayType(0)}
          />
          <Divider flexItem />
          <StatusItem
            label="자가점검 미제출"
            count={0}
            bgcolor={delay_type == 1 ? "lightblue" : "lightgrey"}
            onClick={() => setDelayType(1)}
          />
          <Divider flexItem />
          <StatusItem
            label="보완자료 미제출"
            count={0}
            bgcolor={delay_type == 2 ? "lightblue" : "lightgrey"}
            onClick={() => setDelayType(2)}
          />
    ...
    return (
      ...
      >
        ...
        <Collapse in={open} timeout="auto" unmountOnExit>
          <StatusItem
            label="최초 검수"
            count={checkingDetail.filter((x: any) => x.state == 1).length}
            bgcolor={checking_type == 0 ? "lightblue" : "lightgrey"}
            onClick={() => setCheckingType(0)}
          />
          <Divider flexItem />
          <StatusItem
            label="이행 검수"
            count={checkingDetail.filter((x: any) => x.state == 1).length}
            bgcolor={checking_type == 1 ? "lightblue" : "lightgrey"}
            onClick={() => setCheckingType(1)}
          />

          <Typography variant="h6" sx={{ mt: 1 }}>
            총{" "}
            {
              checkingDetail.filter((x: any) => x.state == checking_type + 1)
                .length
            }{" "}
            건
          </Typography>
          <List>
            {checkingDetail
              .filter((x: any) => x.state == checking_type + 1)
              .map((item, index) => (
                <ListItem
                  key={index}
                  button
                ...
                >
                  ...
                  <Typography
                    ...
                    style={{ marginLeft: "auto" }}
                  >
                    {item.self_check_date == new Date().toDateString()
                      ? "!New"
                      : ""}
                  </Typography>
                </ListItem>
      ...
      >
        ...
        <Collapse in={open} timeout="auto" unmountOnExit>
          <StatusItem
            label="현장 점검 일정 수립"
            count={
              scheduleCheckDetail.filter(
                (x: any) => x.state == 1 && x.sub_state == 1
              ).length
            }
            bgcolor={schedule_type == 0 ? "lightblue" : "lightgrey"}
          ...
          <StatusItem
            ...
            count={
              scheduleCheckDetail.filter(
                (x: any) => x.state == 1 && x.sub_state == 2
              ).length
            }
            bgcolor={schedule_type == 1 ? "lightblue" : "lightgrey"}
          ...
          <StatusItem
            ...
            count={
              scheduleCheckDetail.filter(
                (x: any) => x.state == 2 && x.sub_state == 1
              ).length
            }
            bgcolor={schedule_type == 2 ? "lightblue" : "lightgrey"}
          ...
          <StatusItem
            ...
            count={
              scheduleCheckDetail.filter(
                (x: any) => x.state == 2 && x.sub_state == 2
              ).length
            }
            bgcolor={schedule_type == 3 ? "lightblue" : "lightgrey"}
            onClick={() => setScheduleType(3)}
          />
          <Typography variant="h6" sx={{ mt: 1 }}>
            총{" "}
            {
              scheduleCheckDetail.filter((x: any) =>
                schedule_type == 0
                  ? x.state == 1 && x.sub_state == 1
                  : schedule_type == 1
                  ? x.state == 1 && x.sub_state == 2
                  : schedule_type == 2
                  ? x.state == 2 && x.sub_state == 1
              ...
              .map((item, index) => (
                <ListItem
                  ...
                  onClick={async () => {
                    ...
                    setAnnualPersonalInformation(
                      data.annualPersonalInformation
                    );
  ...
  const TopCard: React.FC<{ bgcolor: any; title: any; count: number }> = ({
    bgcolor,
    title,
    count,
  }) => {
      ...
      <Box>
        <Box display={"flex"} sx={{ margin: 1 }} alignItems="center">
          ...
          >
            {admins.map((x, i) => {
              return (
                <MenuItem key={i} value={x.user_id}>
                  {x.name}
                </MenuItem>
              );
            })}
          ...
          >
            {projects.map((x, i) => {
              return (
                <MenuItem key={i} value={x.project_id}>
                  {x.name}
                </MenuItem>
              );
            })}
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={6} lg={6}>
            ...
            <LinearProgress
              variant="determinate"
              value={totalCount == 0 ? 0 : (totalCompCount1 / totalCount) * 100}
              sx={{ height: "50px", borderRadius: "10px", marginTop: "10px" }}
            />
            <Grid container spacing={2} mt={1}>
              ...
              <TopCard
                title={"검수 중"}
                count={checkingCount1}
                bgcolor={"success"}
              />
              <TopCard
                title={"검수 완료"}
                count={completeCount1}
                bgcolor={"info"}
              />
            </Grid>
          </Grid>
          <Grid item xs={6} lg={6}>
            ...
            <LinearProgress
              variant="determinate"
              value={totalCount == 0 ? 0 : (totalCompCount2 / totalCount) * 100}
              sx={{ height: "50px", borderRadius: "10px", marginTop: "10px" }}
            />
            ...
            <Grid container spacing={2} mt={1}>
              ...
              <TopCard
                title={"검수 중"}
                count={checkingCount2}
                bgcolor={"success"}
              />
              <TopCard
                title={"검수 완료"}
                count={completeCount2}
                bgcolor={"info"}
              />
            </Grid>
          </Grid>
              ...
              <Box display={"flex"}>
                <Box>
                  ...
                  <Box sx={{ borderBottom: 1, borderRadius: 0, pb: 1 }}>
                    <Typography
                      sx={{ fontSize: 15, fontWeight: "bold", ml: 1, mt: 1 }}
                    >
                      수탁사정보
                    </Typography>
                    ...
                    <Box sx={{ ml: 3, mt: 1 }}>
                      <Box sx={{ display: "flex", mt: "4px" }}>
                        <Typography sx={{ width: 150 }}>위탁 업무</Typography>
                        <Typography>{contractContent}</Typography>
                      </Box>

                      <Box sx={{ display: "flex", mt: "4px" }}>
                        <Typography sx={{ width: 150 }}>주소</Typography>
                        <Typography>{address}</Typography>
                      </Box>

                      <Box sx={{ display: "flex", mt: "4px" }}>
                        <Typography sx={{ width: 150 }}>보안 담당자</Typography>
                        <Typography>{managerName}</Typography>
                      </Box>

                      <Box sx={{ display: "flex", mt: "4px" }}>
                        <Typography sx={{ width: 150 }}>연락처</Typography>
                        <Typography>{managerPhoen}</Typography>
                      </Box>

                      <Box sx={{ display: "flex", mt: "4px" }}>
                        <Typography sx={{ width: 150 }}>이메일</Typography>
                      ...
                      <Box sx={{ display: "flex", mt: "4px" }}>
                        ...
                        <Typography>{privacyHandlers}명</Typography>
                      </Box>
                      ...
                      <Box sx={{ display: "flex", mt: "4px" }}>
                        ...
                        <Typography>연간 {yearlyPrivacyHandle}만건</Typography>
                      </Box>
                      ...
                      <Box sx={{ display: "flex", mt: "4px" }}>
                        ...
                        <Typography>재위탁사명/제3자제공 업체명</Typography>
                      </Box>
                      <Box
                        sx={{
                          display: "flex",
                  ...
                  <Box>
                    ...
                    <Box sx={{ ml: 3, mt: 1 }}>
                      <Box sx={{ display: "flex", mt: "4px" }}>
                        <Typography sx={{ width: 150 }}>현업 부서</Typography>
