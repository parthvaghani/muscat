...
export default function EcomProductList() {
  ...
  async function fetchProjectList() {
    try {
      ...
      if (response.data.length > 0) setProjectId(response.data[0].id);
    } catch (error) {
  ...
  return (
    <PageContainer>
      ...
      <Box sx={{ mb: 2, display: "flex", alignItems: "center" }}>
        ...
        >
          <MenuItem value={-1} key={1000000}>
            전체
          </MenuItem>
          {consigneeList.map((x: any, index: number) => {
            return (
              <MenuItem value={x.company_id} key={index}>
                {x.company_name}
              </MenuItem>
            );
          })}
      ...
      {type == "individual" && (
        <StatusIndividual
          consignor={
            consignorList.filter((x: any, index: number) => {
