...
export default function StatusAll({ consigneeList }: { consigneeList: any }) {
  ...
  const [companyScoresPercentage, setCompanyScoresPercentage] = useState<
    [string, number][]
  >([]); //평균대비 우수/취약 수탁사
  ...
  useEffect(() => {
    ...
    setTotalCompCount1(
      consigneeList.filter((x: any) => x.state == 1 && x.sub_state == 3).length
    ); //수정필요
    setTotalCompCount2(
      consigneeList.filter((x: any) => x.state == 2 && x.sub_state == 3).length
    ); //수정필요
    setExceptTotalCount(consigneeList.filter((x: any) => x.state == 3).length);
    setExceptCount1(
      consigneeList.filter((x: any) => x.state == 3 && x.except_type == 1)
        .length
    );
    setExceptCount2(
      consigneeList.filter((x: any) => x.state == 3 && x.except_type == 2)
        .length
    );
    setExceptCount3(
      consigneeList.filter((x: any) => x.state == 3 && x.except_type == 4)
        .length
    );

    const uniqueCompanyIds = new Set(
      consigneeList.map((item: any) => item.company_id)
    );
    ...
    const numberOfFailedCheck1 = consigneeList.reduce(
      (count: any, item: any) => {
        if (item.first_check_data) {
          const firstCheckData = JSON.parse(item.first_check_data);
          const failedChecks = firstCheckData.filter(
            (check: any) => check.check_result === "N"
          );
    ...
    const numberOfFailedCheck2 = consigneeList.reduce(
      (count: any, item: any) => {
    ...
    const countSystemUsageStatusYes = consigneeList.reduce(
      (count: any, item: any) => {
        if (item.status) {
          const status = JSON.parse(item.status);
          console.log(status);
          if (status.systemUsageStatus === "yes") return count + 1;
        }
        return count;
      },
      0
    ...
    const countSystemUsageStatusNo = consigneeList.reduce(
      (count: any, item: any) => {
        if (item.status) {
          const status = JSON.parse(item.status);
          console.log(status);
          if (status.systemUsageStatus === "no") return count + 1;
        }
        return count;
      },
      0
    ...
    const retrustCount1 = consigneeList.reduce((count: any, item: any) => {
      if (item.status) {
        const status = JSON.parse(item.status);
        if (status.retrustStatus === "yes") return count + 1;
      }
      return count;
    }, 0);
    ...
    const retrustCount2 = consigneeList.reduce((count: any, item: any) => {
      if (item.status) {
        const status = JSON.parse(item.status);
        if (status.retrustStatus === "no") return count + 1;
      }
      return count;
    }, 0);
    ...
    consigneeList.forEach((item: any) => {
      ...
      if (item.imp_check_data) {
        totalCountByCompany[companyName] =
          (totalCountByCompany[companyName] || 0) + 1;
      } else {
        totalCountByCompany[companyName] =
          totalCountByCompany[companyName] || 0;
      }
    });
    console.log("@@@@@@@@@@@222222");
    ...
    consigneeList.forEach((item: any) => {
      ...
      if (item.imp_check_data) {
        ...
        const countY: number = impCheckData.filter(
          (item: any) => item.result === "Y"
        ).length;
    ...
    const sortedCompanies: [string, number][] = Object.entries(
      companyScores
    ...
    for (const [area] of Object.entries(firstCountByArea)) {
      ...
      const totalCountByArea = consigneeList.reduce(
        (acc: number, curr: any) => {
          if (curr.first_check_data) {
            const data = JSON.parse(curr.first_check_data);
            const areaCount = data.filter(
      ...
      firstScorePercentByArea[area] = Math.floor(
        (firstCountByArea[area] * 100) / totalCountByArea
      );
    }
    ...
    for (const [area] of Object.entries(impCountByArea)) {
      ...
      // 해당 area에 대한 N의 백분율을 계산하여 저장
      impScorePercentByArea[area] = Math.floor(
        (impCountByArea[area] * 100) / totalCountByArea
      );
    }
    setImpScoreByArea(impScorePercentByArea);

    // 각 domain별로 N인 개수를 카운트하는 객체
    const countCheckResultNByDomain = consigneeList.reduce(
      (acc: any, curr: any) => {
        if (curr.imp_check_data) {
          const data = JSON.parse(curr.imp_check_data);
          data.forEach((item: any) => {
    ...
    for (const [domain] of Object.entries(countCheckResultNByDomain)) {
      ...
      const totalCountByDomain = consigneeList.reduce(
        (acc: number, curr: any) => {
          if (curr.imp_check_data) {
            const data = JSON.parse(curr.imp_check_data);
            const domainCount = data.filter(
      ...
      countResultNByDomain[domain] = Math.floor(
        (countCheckResultNByDomain[domain] * 100) / totalCountByDomain
      );
    }

    // 각 domain별로 Y인 개수를 카운트하는 객체
    const countCheckResultYByDomain = consigneeList.reduce(
      (acc: any, curr: any) => {
        if (curr.first_check_data) {
          const data = JSON.parse(curr.first_check_data);
          data.forEach((item: any) => {
    ...
    for (const [domain] of Object.entries(countCheckResultYByDomain)) {
      ...
      countResultYByDomain[domain] = Math.floor(
        (countCheckResultYByDomain[domain] * 100) / totalCountByDomain
      );
    }
    ...
    consigneeList.forEach((item: any) => {
      ...
      if (item.imp_check_data) {
        ...
        const score = Math.floor((countY / totalCount) * 100);
        totalCountByWorkName[workName] =
          (totalCountByWorkName[workName] || 0) + score;
      } else {
    ...
    const countCheckResultNByStatus: Record<string, number> =
      consigneeList.reduce((acc: any, curr: any) => {
        if (curr.imp_check_data) {
          const data = JSON.parse(curr.imp_check_data);
          data.forEach((item: any) => {
    ...
    for (const [status, count] of Object.entries(countCheckResultNByStatus)) {
      ...
      const totalCountByStatus = consigneeList.reduce(
        (acc: number, curr: any) => {
          if (curr.imp_check_data) {
            const data = JSON.parse(curr.imp_check_data);
            const statusCount = data.filter((item: any) => {
              const itemStatus = JSON.parse(
                item.status
              ).annualPersonalInformationdms;
      ...
      countResultNByStatus[status] = Math.floor(
        (count * 100) / totalCountByStatus
      );
    }
    ...
    console.log(
      "status.annualPersonalInformation별로 check_result가 N인 개수:",
      countResultNByStatus
    );

    //전년도 대비 점검 영역별 점수 추이
    const countCheckResultYByAnnualArea = consigneeList.reduce(
      (acc: any, curr: any) => {
        if (curr.imp_check_data) {
          const data = JSON.parse(curr.imp_check_data);
          data.forEach((item: any) => {
    ...
    for (const [area] of Object.entries(countCheckResultYByAnnualArea)) {
      ...
      // 해당 area에 대한 N의 백분율을 계산하여 저장
      countResultYByAnnualArea[area] = Math.floor(
        (countCheckResultYByAnnualArea[area] * 100) / totalCountByArea
      );
    }
    setCountYByAnnualArea(countResultYByAnnualArea);
    //.. 2024년도
    const countCheckResultNByAnnualArea = consigneeList.reduce(
      (acc: any, curr: any) => {
        if (curr.imp_check_data) {
          const data = JSON.parse(curr.imp_check_data);
          data.forEach((item: any) => {
    ...
    for (const [area] of Object.entries(countCheckResultNByAnnualArea)) {
      ...
      // 해당 area에 대한 N의 백분율을 계산하여 저장
      countResultNByAnnualArea[area] = Math.floor(
        (countCheckResultNByAnnualArea[area] * 100) / totalCountByArea
      );
    }
    ...
    <PageContainer>
      <Grid container>
        <Grid xs={1}>
          ...
          >
            <Typography sx={{ color: "red", fontSize: 20, marginRight: 1 }}>
              77
            </Typography>
        ...
        <Grid xs={10} container>
          ...
          >
            ...
            <Typography>중 최초점검 완료</Typography>
            <Typography
              sx={{
                color: "#4472C4",
                fontSize: 20,
            ...
            <Typography>,점검제외</Typography>
            <Typography
              sx={{ color: "red", fontSize: 20, ml: "5px", fontWeight: "bold" }}
            >
              {exceptTotalCount}건
            </Typography>
            <Typography>(PG사</Typography>
            <Typography sx={{ color: "red", fontSize: 14, ml: "5px" }}>
              {exceptCount1}건
            </Typography>
            <Typography>/인증서 제출</Typography>
            <Typography sx={{ color: "red", fontSize: 14, ml: "5px" }}>
              {exceptCount2}건
            </Typography>
            <Typography>/점검 거부</Typography>
            <Typography sx={{ color: "red", fontSize: 14, ml: "5px" }}>
              {exceptCount3}건
            </Typography>
          ...
          <Grid item xs={6} style={{ padding: 8 }} width={16}>
            <CheckStatus
              numberOfFailedCheck1={failedCheckCount1}
              numberOfFailedCheck2={failedCheckCount2}
          ...
          <Grid item xs={6} style={{ padding: 8 }}>
            <ConsigneeWidget
              scoresPercentage={companyScoresPercentage}
              title={"평균 대비 우수/취약 수탁사 "}
            ></ConsigneeWidget>
          </Grid>
          <Grid item xs={6} style={{ padding: 8 }}>
            <InnerSystemEnable
              systemUsageStatusCount1={systemUsageStatusCount1}
              systemUsageStatusCount2={systemUsageStatusCount2}
            ></InnerSystemEnable>
          </Grid>
          <Grid item xs={6} style={{ padding: 8 }}>
            <RetrustStatus
              retrustCount1={retrustCount1}
              retrustCount2={retrustCount2}
            ></RetrustStatus>
          </Grid>
          <Grid item xs={6} style={{ padding: 8 }}>
            <RadialbarChart
              countYByArea={firstScoreByArea}
              countNByArea={impScoreByArea}
              title={"최초/이행 영역별 평균 점수"}
            ></RadialbarChart>
          </Grid>
          <Grid item xs={6} style={{ padding: 8 }}>
            <RadialbarChart
              countYByArea={firstScoreByDomain}
              countNByArea={impScoreByDomain}
              title={"최초/이행 분야별 평균 점수"}
            ></RadialbarChart>
          </Grid>
          <Grid item xs={5} style={{ padding: 8 }}>
            <ConsigneeWidget
              scoresPercentage={Object.entries(workNameScoresPercentage)}
              title={"업종에 따른 평균 점수"}
            ></ConsigneeWidget>
          </Grid>
          <Grid item xs={5} style={{ padding: 8 }}>
            <ConsigneeWidget
              scoresPercentage={Object.entries(countResultNByStatus)}
              title={"개인정보 처리량에 따른 평균 점수"}
            ></ConsigneeWidget>
            <ConsigneeWidget
              scoresPercentage={Object.entries(countYByAnnualArea)}
              title={"전년도 대비 점검 영역별 점수 추이"}
            ></ConsigneeWidget>
          </Grid>
          <Grid item xs={4} lg={4}>
            <DashboardCard
