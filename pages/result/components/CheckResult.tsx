...
const CheckResult = ({
  ...
  React.useEffect(() => {
    ...
    if (consigneeData.project_id) fetchChecklist();
  }, [consigneeData]);

  React.useEffect(() => {
    if (
      userType == 1 &&
      consigneeData.state == 1 &&
      consigneeData.sub_state == 1
    ) {
      //- 자가점검 중 :: 수탁사에만 해당
      let newData: any[] = [];
      checklistData.map((x: any) => {
        let d = JSON.parse(consigneeData.first_check_consignee_temp_data).find(
          (a: any) => a.id == x.id
        );
        x = { ...x, ...d };
        console.log(x);
      ...
      setCheckDataArray(newData);
    } else if (consigneeData.state == 1 && consigneeData.sub_state == 2) {
      //- 검수중
      console.log(JSON.parse(consigneeData.first_check_data));
      setCheckDataArray(JSON.parse(consigneeData.first_check_data));
    } else if (consigneeData.state == 1 && consigneeData.sub_state == 3) {
      //- 최초점검완료단계
      if (userType != 1) {
        setCheckDataArray(
          JSON.parse(consigneeData.first_check_admin_temp_data)
        );
      }
    } else if (consigneeData.state == 2 && consigneeData.sub_state == 1) {
      //- 보완자료요청, 이행점검대기단계
      if (userType == 1) {
        //수탁사인 경우
        setCheckDataArray(
          JSON.parse(consigneeData.imp_check_consignee_temp_data)
        );
      } else {
        //어드민인 경우
  ...
  const onSave = async () => {
    ...
    } else if (consigneeData.state == 1 && consigneeData.sub_state == 1) {
      //- 자가점검 중 :: 수탁사에만 해당
      response = await axiosPost(`${API_URL}/project_detail/Update`, {
        id: consigneeData.id,
  ...
  const consigneeSubmit = async () => {
    ...
    if (checkExcept == true) {
      ...
      let first_check_miss_count = checkDataArray.filter(
        (item: any) => item["check_result"] === "N"
      ).length;
      ...
      let imp_check_miss_count = checkDataArray.filter(
        (item: any) => item["result"] === "N"
      ).length;
  ...
  return (
    <Box>
      <Box display={"flex"} justifyContent={"flex-end"}>
        <Typography sx={{ mr: 3 }}>
          {consigneeData.company_name}상세점검결과
        </Typography>
        <Typography>점검일{consigneeData.first_check_date}</Typography>
      </Box>
      {(userType == 0 || userType == 3) &&
      !(consigneeData.sub_state == 1 && consigneeData.state == 1) ? ( //어드민이면서 자가점검이 아닐 경우에만 현시
        <>
          <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
            ...
            >
              {consigneeData.sub_state == 2 && consigneeData.state == 2
                ? "최종 저장"
                : "검수 완료"}
            </Button>
          </Box>
                ...
                <FormControlLabel
                  ...
                  control={
                    <CustomCheckbox
                      ...
                      checked={checkExcept}
                      onChange={(e: any) => {
                        setCheckExcept(e.target.checked);
                      }}
                      inputProps={{
      ...
      <Box display={"flex"} justifyContent={"flex-start"} alignItems={"center"}>
        ...
        <a
          href={`${API_URL}/project_detail/Attachment?file_name=${encodeURIComponent(
            project_attachment != ""
              ? project_attachment
              : consigneeData.project_attachment
          )}`}
        >
          {project_attachment != ""
            ? project_attachment
            ...
            {checklistData.map((row: any, rowIndex) => (
              <TableRow key={row.id}>
                ...
                >
                  {expandedRow === rowIndex &&
                  !(
                    consigneeData.sub_state == 2 &&
                    consigneeData.state == 2 &&
                    userType == 1 &&
                    (checkDataArray.filter((x: any) => x.id == row.id).length ==
                    0
                      ? false
                      : checkDataArray.filter((x: any) => x.id == row.id)[0]
                          .lock == "Y")
                  ) ? (
                    <CustomSelect
                      ...
                      sx={{ width: 70 }}
                      value={
                        checkDataArray.filter((x: any) => x.id == row.id)
                          .length == 0
                          ? ""
                          : checkDataArray.filter((x: any) => x.id == row.id)[0]
                              .self_check_result
                      ...
                      style={{
                        ...
                        color:
                          checkDataArray.filter((x: any) => x.id === row.id)
                            ...
                            : checkDataArray.filter(
                                (x: any) => x.id === row.id
                              )[0].self_check_result === "Y"
                            ? "blue"
                            : checkDataArray.filter(
                                (x: any) => x.id === row.id
                              )[0].self_check_result === "N"
                            ? "red"
                            : "black",
                      }}
                    />
                ...
                >
                  ...
                  !(
                    ...
                    <Box>
                      <Box sx={{ mb: 2 }}>
                        <a
                          href={`${API_URL}/checkinfo/Attachment?file_name=${encodeURIComponent(
                            row.filename
                          )}`}
                        >
                          {row.filename}
                      ...
                      <a
                        href={`${API_URL}/checkinfo/Attachment2?file_name=${encodeURIComponent(
                          checkDataArray.filter((x: any) => x.id == row.id)
                            .length == 0
                            ? ""
                            : checkDataArray.filter(
                      ...
                      <CustomSelect
                        ...
                        onChange={(event: any) => {
                          ...
                          checkDataArray.map((x) => {
                            if (x.id == row.id) {
                              x.attachment =
                                event.target.value == "Y" ? attachment : "";
                            }
                            newData.push(x);
                          });
                          setCheckDataArray(newData);
