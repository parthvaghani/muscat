...
let goArray = [Go0, Go1, Go2, Go3, Go4, Go5, Go6, Go7];
const BCrumb = [
  {
    to: "/",
    title: "메인",
...
export default function StatusIndividual({
  consignor,
  consigneeData,
}: {
  consignor: string;
  ...
  React.useEffect(() => {
    ...
    if (countResultYByDomain.first_check_data) {
      ...
      data.forEach((item: any) => {
        ...
        if (item.check_result === "Y" && item.domain) {
          countResultYByDomain[item.domain] =
            (countResultYByDomain[item.domain] || 0) + 1;
        }
      });
    }
    ...
    for (const [domain] of Object.entries(countResultYByDomain)) {
      ...
      countResultByDomain[domain] = Math.floor(
        (countResultYByDomain[domain] * 100) / areaCount
      );
    }
    //////////////////////////////////
    ...
    if (countResultNByDomain.first_check_data) {
      ...
      data.forEach((item: any) => {
        // 현재 요소의 area를 키로 사용하여 그룹화하고, check_result가 N인 경우 카운트 증가
        if (item.check_result === "Y" && item.domain) {
          countResultNByDomain[item.domain] =
            (countResultNByDomain[item.domain] || 0) + 1;
        }
      });
    }
    ...
    for (const [domain] of Object.entries(countResultNByDomain)) {
      ...
      // 해당 area에 대한 N의 백분율을 계산하여 저장
      countResultByDomain2[domain] = Math.floor(
        (countResultByDomain2[domain] * 100) / areaCount
      );
    }
      ...
      <Grid container>
        <Grid item xs={1}>
          ...
          <Box
            ...
            sx={{
              backgroundColor: type == "status" ? "#FFFFFF" : "#D9D9D9",
              textAlign: "center",
              borderRadius: 0,
              borderColor: "#A5A5A5",
              border: 1,
          ...
          <Box
            ...
            sx={{
              backgroundColor: type == "check" ? "#FFFFFF" : "#D9D9D9",
              textAlign: "center",
              borderRadius: 0,
              borderColor: "#A5A5A5",
              border: 1,
          ...
          >
            <Typography sx={{ fontSize: 16 }}>
              {consigneeData.state == 2 && consigneeData.sub_state == 2
                ? "점검 결과"
                : "점검 수행"}
            </Typography>
          </Box>
          ...
          {type == "status" ? (
            <Grid item xs={12} container>
              ...
              {stage >= 1 && stage <= 7 && (
                ...
                >
                  <Typography
                    sx={{ fontSize: 24, color: "#2F5597", fontWeight: "bold" }}
                  >
                    {" "}
                    &gt;&gt;&gt; 점검 진행중입니다. &gt;&gt;&gt;{" "}
                  </Typography>
